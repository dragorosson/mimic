...
class ValkyrieAPITests(SynchronousTestCase):
    ...
    TODO: add some negative tests, such as non-existent endpoints and contacts with no permissions (after confirming what code and content Valkyrie returns on such conditions)
    """
    ...
    def test_post_auth_token_to_login_endpoint(self):
        ...
        (response, content) = self.successResultOf(json_request(self, self.root, "POST", self.url + "/login", data))
    ...
    def test_post_auth_token_to_login_user_endpoint(self):
        ...
        """
        data = {"something": "anything"}
        (response, content) = self.successResultOf(json_request(self, self.root, "POST", self.url + "/login_user", data))
        self.assertEqual(200, response.code)
    ...
    def test_get_devices_effective_permissions(self):
        ...
        """
        (response, content) = self.successResultOf(json_request(self, self.root, "GET", self.url + "/account/1234/permissions/contacts/devices/by_contact/12/effective"))
        self.assertEqual(200, response.code)
    ...
    def test_get_accounts_effective_permissions(self):
        ...
        """
        (response, content) = self.successResultOf(json_request(self, self.root, "GET", self.url + "/account/1234/permissions/contacts/accounts/by_contact/12/effective"))
        self.assertEqual(200, response.code)
        self.assertTrue(content["contact_permissions"])
    ...
    def test_effective_permissions(self):
        ...
        """
        (response, content) = self.successResultOf(json_request(self, self.root, "GET", self.url + "/account/5678/permissions/contacts/devices/by_contact/78/effective"))
        self.assertEqual(200, response.code)
        self.assertTrue(content["contact_permissions"])
        self.assertEqual(len(content["contact_permissions"]),1)
